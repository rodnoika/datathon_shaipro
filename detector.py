...
def summarize_incidents(findings: pd.DataFrame, top_k:int=20) -> pd.DataFrame:
    agg = (findings.groupby("src_ip")
                   ...
                   .reset_index())
    return agg.sort_values(["rule_hits","max_if_score"], ascending=[False,False]).head(top_k)

def summarize_firewall_incidents(logs_fw: pd.DataFrame, top_k:int=20) -> pd.DataFrame:
    blocked = logs_fw[logs_fw["action"].str.lower() == "deny"].copy()
    if blocked.empty:
        return pd.DataFrame(columns=["src_ip","denies","last_seen"])
    agg = (blocked.groupby("src_ip")
                  .agg(denies=("src_ip","size"),
                       last_seen=("timestamp","max"))
                  .reset_index()
           )
    return agg.sort_values("denies", ascending=False).head(top_k)
